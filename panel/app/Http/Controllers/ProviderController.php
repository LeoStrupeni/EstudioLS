...
class ProviderController extends Controller
{
    ...
    {        
        ...
        $permissions = Session::get('user')['permissions']['providers'];
    ...
    public function update(Request $request, $id)
    {
        ...
        $datos = array();
        if(isset($request->first_name) && $request->first_name != $provider->first_name){
        ...
        if(isset($request->last_names) && $request->last_names != $provider->last_names){
            $datos['last_names'] = $request->last_names;
        }
        if(isset($request->type_doc) && $request->type_doc != $provider->type_doc){
            $request->validate(['type_doc' => ['required']],
                [ 'required' => 'El campo es requerido.']
            );
            $datos['type_doc'] = $request->type_doc;
        }
        if(isset($request->num_doc) && $request->num_doc != $provider->num_doc){
            $request->validate(['num_doc' => ['required']],
                [ 'required' => 'El campo es requerido.']
            );
            $datos['num_doc'] = $request->num_doc;
        }
        if(isset($request->email) && $request->email != $provider->email){
        ...
        if(isset($request->phone1) && $request->phone1 != $provider->phone1){
            $datos['phone1'] = $request->phone1;
        }
        if(isset($request->phone2) && $request->phone2 != $provider->phone2){
            $datos['phone2'] = $request->phone2;
        }
        if(isset($request->country) && $request->country != $provider->country){
            $datos['country'] = $request->country;
        }
        if(isset($request->state) && $request->state != $provider->state){
            $datos['state'] = $request->state;
        }
        if(isset($request->city) && $request->city != $provider->city){
            $datos['city'] = $request->city;
        }
        if(isset($request->address_street) && $request->address_street != $provider->address_street){
            $datos['address_street'] = $request->address_street;
        }
        if(isset($request->address_nro) && $request->address_nro != $provider->address_nro){
            $datos['address_nro'] = $request->address_nro;
        }
        if(isset($request->address_apartament) && $request->address_apartament != $provider->address_apartament){
            $datos['address_apartament'] = $request->address_apartament;
        }
        if(isset($request->address_detail) && $request->address_detail != $provider->address_detail){
            $datos['address_detail'] = $request->address_detail;
        }
        
        if(count($datos) > 0){
            Provider::where('id',$id)->update($datos);
        }
