$(document).ready(function() {
                ...
                if(data.datos.length == 0){$('#modal-body-show-rolpermissions-sindatos').removeClass('d-none');}
                    ...
                    $.each(data.datos, function (key, val) {
                        ...
                        body += `<tr id="${val.general}">
                            <td class="align-middle text-start ps-3">${val.traslate}</td>
    ...
    $('body').on('click','.delete',function(){ 
        ...
        Swal.fire({
            title: "Borrar Rol",
            html: "Esta seguro que desea eliminar el rol "+$(this).data('name')+"?<br>No podrá revertir el cambio.",
            type: "question",
...
function tableregister(data, page, callpaginas, url_query){
    ...
    $.each(data.datos, function (key, val) {
        ...
        body += `<tr id="${val.id}">
            ...
            <td class="align-middle">
                <button class="btn w-100 p-0 ${val.name != 'sistema' ? (val.estatus == 'Activo' ? 'btn-success' : 'btn-danger') : 'btn-secondary'} btn-sm ${data.permissions.includes('update') && val.name != 'sistema' ? 'estatus' : ''}" data-id="${val.id}">${val.estatus}</button>
            </td>
        ...
        bodysmall += `<div class="col-11 mb-3 mx-3">
            ...
            <div class="row">
                ...
                <div class="col-6 border p-1 text-center">
                    <button class="btn w-75 p-0 ${val.name != 'sistema' ? (val.estatus == 'Activo' ? 'btn-success' : 'btn-danger') : 'btn-secondary'} btn-sm ${data.permissions.includes('update') && val.name != 'sistema' ? 'estatus' : ''}" data-id="${val.id}">${val.estatus}</button>
                </div>
            </div>
            <div class="row">
